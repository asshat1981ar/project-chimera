...
- `tools/huggingface/` contains developer-only CLI helpers for evaluating and probing the existing NPC portrait-generation workflow.
- These tools use `HF_TOKEN` for Hugging Face API access and must not change app Gradle configuration, Android source, generated assets, or the app-facing `HUGGING_FACE_TOKEN` property.
